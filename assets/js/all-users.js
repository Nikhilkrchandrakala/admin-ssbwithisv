...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderTable(studentsList) {
        ...
        studentTableBody.innerHTML = studentsList.map(s => {
            ...
            const roleClass = roleColors[s.role] || "bg-secondary";
            const roleHtml = `<span class="badge ${roleClass} text-uppercase" style="font-size: 0.75rem;">${escapeHtml(s.role || "unknown")}</span>`;
            ...
            if (s.clinicalStage) {
                ...
                badgesContainer = stages.map(st => `<span class="badge bg-dark border border-secondary text-light px-2 py-1 small me-1 mb-1">${escapeHtml(stageTitles[st] || st)}</span>`).join("");
            } else {
                badgesContainer = `<span class="text-muted small">No Course Access</span>`;
            }
            ...
            return `
                <tr>
                    ...
                    <td style="text-align: center;">
                        <button class="action-btn" title="View Profile" onclick="openStudentModal('${s._id}')">
    ...
    function applyFilters() {
        ...
        filteredStudents = allStudents.filter(s => {
            ...
            // For admin dropdown we combine admin & superadmin
            if (role === "admin" && (s.role === "admin" || s.role === "superadmin")) {
